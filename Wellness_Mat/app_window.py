...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.info_layout.addWidget(QtWidgets.QLabel('Select Type of Exercise:'))
    ...
    def start_exercise(self):
        ...
        if "Walking" in exercise_type:
    
            # Initialize walking heatmap
            self.heatmap_widget = PressureMapAppTreadmill(visual_queue)
            self.heatmap_layout.addWidget(self.heatmap_widget)

            # Start data collection and visualization for walking
            self.data_process = multiprocessing.Process(target=data_collection_process, args=(self.running,))
            self.visual_process = multiprocessing.Process(target=process_visualization, args=(visual_queue, data_queue,))
            self.data_process.start()
            self.visual_process.start()
        else:
            self.heatmap_widget = PressureMapAppSingle(data_queue)  # Single mat widget for non-walking exercises
            self.data_process = multiprocessing.Process(target=get_mat_pressures, args=(data_queue,))
            self.data_process.start()
    ...
    def stop_exercise(self):
        ...
        # Stop the data collection process if it's running
        ...
        for i in reversed(range(self.heatmap_layout.count())):
            widget = self.heatmap_layout.itemAt(i).widget()
            if widget is not None:
                widget.deleteLater()  # Delete the widget to free up resources

        # Re-add the heatmap placeholder after clearing
        self.heatmap_placeholder = QtWidgets.QLabel(self)
        self.heatmap_placeholder.setText("Heatmap will appear here")
        self.heatmap_placeholder.setAlignment(QtCore.Qt.AlignCenter)
