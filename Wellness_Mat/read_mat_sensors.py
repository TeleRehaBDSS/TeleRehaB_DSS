...
from frontend_widget import data_queue
...
#   while True:
  ...
  data_export.export_to_json(current_map, 'refactoring') # un-comment to create json file with the appended maps (excercise)
...
def data_collection_process(running_flag):
    ...
    #for pr in data_process:
    #    pr.join()

    time.sleep(80)
    export_queue_to_csv(export_queue, f"DATA_{time.time()}.csv");
